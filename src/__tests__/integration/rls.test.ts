...
interface Ctx {
  ...
  evA: string; evB: string;
  regA1: string; regB1: string; regA2: string;
  superAdmin: TestUser; adminA: TestUser; adminB: TestUser;
  staffA: TestUser; plain: TestUser; attendee: TestUser;
}
...
async function cleanup(ctx: Partial<Ctx>) {
  ...
  if (ctx.regA1 || ctx.regB1 || ctx.regA2) {
    await a.from("event_registrations").delete().in(
      "id", [ctx.regA1, ctx.regB1, ctx.regA2].filter(Boolean) as string[],
    );
  }
  if (ctx.evA || ctx.evB) {
    await a.from("events").delete().in(
      "id", [ctx.evA, ctx.evB].filter(Boolean) as string[],
    );
  }
  const userIds = [
    ctx.superAdmin?.id, ctx.adminA?.id, ctx.adminB?.id,
    ctx.staffA?.id, ctx.plain?.id, ctx.attendee?.id,
  ...
  if (ctx.orgA || ctx.orgB) {
    await a.from("organizations").delete().in(
      "id", [ctx.orgA, ctx.orgB].filter(Boolean) as string[],
    );
  }
}
...
d("RLS integration: profiles & event_registrations", () => {
  ...
  beforeAll(async () => {
    ...
    // 2 events
    const evA = newId(), evB = newId();
    const evInsert = await admin.from("events").insert([
      { id: evA, organization_id: orgA, name: `${TAG}_evt_a`, slug: `${TAG}-evt-a-${Date.now()}`,
        starts_at: new Date().toISOString(), ends_at: new Date(Date.now() + 86400000).toISOString(),
        status: "published" },
      { id: evB, organization_id: orgB, name: `${TAG}_evt_b`, slug: `${TAG}-evt-b-${Date.now()}`,
        starts_at: new Date().toISOString(), ends_at: new Date(Date.now() + 86400000).toISOString(),
        status: "published" },
    ]);
    ...
    // 3 registrations
    const regA1 = newId(), regB1 = newId(), regA2 = newId();
    const regInsert = await admin.from("event_registrations").insert([
      { id: regA1, event_id: evA, full_name: "Anon A", email: `${TAG}_anon_a@test.local`, status: "confirmed" },
      { id: regB1, event_id: evB, full_name: "Anon B", email: `${TAG}_anon_b@test.local`, status: "confirmed" },
      { id: regA2, event_id: evA, full_name: "Attendee", email: attendee.email, status: "confirmed", user_id: attendee.id },
    ]);
    if (regInsert.error) throw regInsert.error;

    ctx = { admin, orgA, orgB, evA, evB, regA1, regB1, regA2,
      superAdmin, adminA, adminB, staffA, plain, attendee };
  }, 60_000);

  afterAll(async () => { await cleanup(ctx ?? {}); }, 30_000);

  // ============ profiles SELECT ============
  describe("profiles SELECT", () => {
    it("super_admin sees the 6 test profiles", async () => {
      const { data, error } = await ctx.superAdmin.client
        .from("profiles").select("id").like("email", `${TAG}%`);
  ...
  describe("profiles UPDATE", () => {
    it("plain user can change own full_name", async () => {
      const { error } = await ctx.plain.client
        .from("profiles").update({ full_name: "New Name" }).eq("id", ctx.plain.id);
      expect(error).toBeNull();
    });

    it("plain user CANNOT self-assign to an organization", async () => {
      await ctx.plain.client.from("profiles")
        .update({ organization_id: ctx.orgA }).eq("id", ctx.plain.id);
      // Either errors or silently no-ops; either way the value must remain NULL.
      const { data } = await ctx.admin.from("profiles")
        .select("organization_id").eq("id", ctx.plain.id).single();
      expect(data?.organization_id).toBeNull();
    });

    it("plain user CANNOT mutate another user's profile", async () => {
      await ctx.plain.client.from("profiles")
        .update({ full_name: "Hack" }).eq("id", ctx.adminA.id);
      const { data } = await ctx.admin.from("profiles")
        .select("full_name").eq("id", ctx.adminA.id).single();
  ...
  describe("event_registrations UPDATE", () => {
    it("staff A can check-in a registration in own org", async () => {
      const { error } = await ctx.staffA.client.from("event_registrations")
        .update({ status: "checked_in", checked_in_at: new Date().toISOString(), checked_in_by: ctx.staffA.id })
        .eq("id", ctx.regA1);
      expect(error).toBeNull();
    });

    it("staff CANNOT mutate email (immutable column)", async () => {
      await ctx.staffA.client.from("event_registrations")
        .update({ email: "hijack@x.io" }).eq("id", ctx.regA1);
      const { data } = await ctx.admin.from("event_registrations")
        .select("email").eq("id", ctx.regA1).single();
      expect(data?.email).toBe(`${TAG}_anon_a@test.local`);
    });

    it("staff CANNOT mutate event_id (immutable column)", async () => {
      await ctx.staffA.client.from("event_registrations")
        .update({ event_id: ctx.evB }).eq("id", ctx.regA1);
      const { data } = await ctx.admin.from("event_registrations")
        .select("event_id").eq("id", ctx.regA1).single();
      expect(data?.event_id).toBe(ctx.evA);
    });

    it("staff CANNOT rotate qr_token", async () => {
      const before = (await ctx.admin.from("event_registrations")
        .select("qr_token").eq("id", ctx.regA1).single()).data?.qr_token;
      await ctx.staffA.client.from("event_registrations")
        .update({ qr_token: crypto.randomUUID() }).eq("id", ctx.regA1);
      const after = (await ctx.admin.from("event_registrations")
        .select("qr_token").eq("id", ctx.regA1).single()).data?.qr_token;
      expect(after).toBe(before);
    });

    it("org_admin A CANNOT update a registration of org B", async () => {
      await ctx.adminA.client.from("event_registrations")
        .update({ status: "cancelled" }).eq("id", ctx.regB1);
      const { data } = await ctx.admin.from("event_registrations")
        .select("status").eq("id", ctx.regB1).single();
      expect(data?.status).toBe("confirmed");
    });

    it("plain user CANNOT update any registration", async () => {
      await ctx.plain.client.from("event_registrations")
        .update({ status: "cancelled" }).eq("id", ctx.regA1);
      const { data } = await ctx.admin.from("event_registrations")
        .select("status").eq("id", ctx.regA1).single();
      expect(data?.status).not.toBe("cancelled");
    });

    it("attendee owner CANNOT modify their own registration", async () => {
      await ctx.attendee.client.from("event_registrations")
        .update({ status: "cancelled" }).eq("id", ctx.regA2);
      const { data } = await ctx.admin.from("event_registrations")
        .select("status").eq("id", ctx.regA2).single();
      expect(data?.status).toBe("confirmed");
    });
  });
  ...
  describe("event_registrations DELETE", () => {
    ...
    it("org_admin A CAN delete a registration of own org", async () => {
      const { error } = await ctx.adminA.client.from("event_registrations").delete().eq("id", ctx.regA1);
      expect(error).toBeNull();
      const { data } = await ctx.admin.from("event_registrations").select("id").eq("id", ctx.regA1);
    ...
    it("super_admin CAN delete any registration", async () => {
      const { error } = await ctx.superAdmin.client.from("event_registrations").delete().eq("id", ctx.regB1);
      expect(error).toBeNull();
      const { data } = await ctx.admin.from("event_registrations").select("id").eq("id", ctx.regB1);
      expect(data?.length).toBe(0);
    });
