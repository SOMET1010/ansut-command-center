...
export function AlertBanner({
  ...
  return (
    <div
      role="alert"
      className={cn("flex items-start gap-3 rounded-md border-l-4 p-3", s.bg, s.border, className)}
    >
      <Icon className={cn("mt-0.5 h-4 w-4 shrink-0", s.text)} />
      <div className="min-w-0 flex-1">
        <div className={cn("text-sm font-semibold", s.text)}>{title}</div>
        {children ? <div className="mt-0.5 text-xs text-foreground/80">{children}</div> : null}
      </div>
